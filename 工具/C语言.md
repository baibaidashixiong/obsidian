...
- Linux中x86 32位memcpy代码底层实现：
  ...
  - **指令部**：
    - 第一条为`rep; movsl`，该指令将n/4个双字（每个4字节）从指向的源地址from复制到指向的目标地址to。rep前缀将movsl指令重复n/4次（因为每次复制4个字节）。
    - 第二条指令`movl %4,%%ecx`，该指令将n的值放入ecx寄存器，后续将被用来检查是否还有剩余字节需要被拷贝。
    - `andl $3,%%ecx`：此指令执行ecx寄存器与值3（二进制11）的逐位AND。其用来提取n/4的余数，表示剩余还要拷贝的字节。
    - `jz 1f`：如果设置了零标志位（表明上一条指令的运行结果为0），则此指令跳转到label 1，表示没有剩余字节可复制。
    - `rep ; movsb`：movsb: move byte from address in `esi` to address in `edi`.将剩余未拷贝的字节拷贝到目标地址中，每次循环ecx-1,直到其为0循环结束。
    - `1:`用于用jz指令跳转的标签。
  - **输出部**` "=&c" (d0), "=&D" (d1), "=&S" (d2)`：有三个约束，对应于操作数%0至%2.其中变量d0为操作数%0.它们分别对应`ecx`, `edi`, `esi`寄存器。`&`符号用来表示它们是仅输出操作数。根据IA32的寄存器规约，d0被隐式的表示为ecx，d1即操作数%1被表示为寄存器edi，d2即%2表示为寄存器esi。
  - **输入部**有四个约束，对应于操作数%3至%6.`"0" (n / 4)`中的0表示操作数%3需要与操作数%0使用同一个寄存器即ecx，且其值为`n/4`，所以一开始传入时ecx值为n/4，rep每次运行其会减1,直到0为止停止循环。`"g" (n)`中的g表示该操作数%4可以由gcc任意分配（空闲的）寄存器，其分配值为n。所以其是该内联汇编语句中唯一一个显式的操作数。`"1" ((long)to)`中的1refer to汇编代码中的输出操作数约束`=&D`，即操作数%5表示寄存器edi，同时使用强制转换long将to指针转换为长类型以匹配movsl和movsb指令所需的操作数。`"2" ((long)from)`中的2表示约束`=&S`，即操作数%6表示寄存器esi。
  - **损坏部**：`"memory"`因为该函数的作用就是进行内存拷贝，所以其需要告知编译器任何内存都可以被该内联汇编语句任意读取或写入，避免gcc对内存进行优化。其可以告诉编译器，汇编代码可能会对输入和输出操作数中**所列参数以外**的参数进行内存读取或写入（例如，访问输入参数之一所指向的内存）。为了确保内存包含正确的值，GCC可能需要在执行asm之前将特定的寄存器值刷入内存。此外，编译器不认为在asm之前从内存中读取的任何数值在该asm之后会保持不变；它会根据需要重新加载它们。该语句可以有效地形成编译器的读/写内存屏障。

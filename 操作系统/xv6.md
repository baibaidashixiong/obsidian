...
### 文件系统
文件系统的出现就是为了实现数据持久化，组织与存储数据。xv6的文件系统包括类unix文件，目录和路径，还有文件中的数据，并将它们储存在虚拟磁盘中来实现持久化。xv6的文件系统需要解决以下几个难题：
- 文件系统需要磁盘上的数据结构来表示目录和文件名，记录保存每个文件内容的块的标识，并记录磁盘上的空闲区域。
- 文件系统必须支持崩溃恢复。也就是说当崩溃（如电源故障）发生时，文件系统必须在重启后仍然能正常工作。其中包含的风险就是系统崩溃可能会导致数据更新和一系列所保留在硬盘上的数据结构不一致（比如说在文件中同时使用并标记为空闲的块）
- 为了使不同的进程可以同时在文件系统中处理，文件系统的代码必须协调维护它们之间保持相对独立。
- 通过直接访问硬盘比访问内存会慢很多，所以文件系统必须维护一个常驻内存的文件块。
#### xv6文件系统的实现
xv6文件系统主要由**七层模型**构成（如下）**磁盘层**主要负责从虚拟硬盘中读取和写入数据；**缓存层**主要保证了写入数据的同步，保证对于某个特定的块，在同一时刻只有一个内核进程可以修改其中的数据；**日志层**允许更高层在一次数据交换中将数据包含在多个数据块中，同时保证这些数据块在遭遇崩溃时可以原子的更新（要么所有数据块都更新到硬盘中要么都不更新）；**结点层**为每一个文件提供一个独一无二的结点编号，然后由多个数据块来保存文件数据；**目录层**为每一个目录实现一个特殊的结点，其内容就是目录条目，每个条目都包含文件名和结点编号；**路径名**层提供路径名，如`/home/user`，并通过递归查找来解析它们；**文件描述符层**抽象了许多unix资源（如管道，设备，文件等）使用文件系统接口可以简化许多应用的编程。
...
<tr > <td>目录名</td></tr> 
 <tr><td>结点Inode</td></tr> 
 <tr><td>路径名</td></tr>
 <tr><td>日志Logging</td></tr> 
 <tr><td>缓存</td></tr> 
 <tr><td>磁盘</td></tr> 
</table>
- 硬盘（硬件资源）通常是由512字节的序列块组成的（也叫分区）。分区0就是开始的512字节，分区1就是接下来的512字节，以此类推。操作系统所抽象的块大小可能会与硬盘的分区大小不同，但是通常来说块大小都是分区大小的数倍。
- 文件系统必须在磁盘中规划存储结点和内容块的地方。xv6将硬盘划分成多个分区来达到此目的（如下图）**块0**为xv6的启动分区；**块1**被称为超级块（superblock）它包含着文件系统的元数据（如文件系统在块中的大小，数据块的数量，结点的数量，以及在log中的块数量）从**块2**开始文件系统中储存的是log信息，在log信息之后储存的是结点信息，每一个块都包含着多个结点信息。在这之后存放的是位图（bitmap）信息，其主要用于存储每一个数据块的使用情况。剩下的磁盘部分就是数据块，每一个数据块要么在位图中被标记为空闲块，要么存储有文件或是目录的内容和信息。超级块主要由`mkfs`程序完成初始化，超级块初始化完成后整个文件系统也就完成了初始化。
![[xv6文件系统的结构.png]]
##### 1.缓存层(Buffer cache layer)
缓存层主要有两个作用：（1）同步对磁盘块的访问，以确保内存中只有一个块副本，并且一次只有一个内核线程使用该副本；（2）常常用到的cache缓冲数据块（xv6中用LRU算法表示）可以从内存中被快速读出，而不用再通过慢速设备硬盘来读取，这一部分代码在`kernel/bio.c`中。

##### 2.日志层(Logging Layer)
- 文件系统设计中最有趣的问题之一就是崩溃恢复。出现此问题的原因是许多文件系统操作都涉及到对磁盘的多次写入，并且在完成其中部分写操作后发生的崩溃会使磁盘上的文件系统处于不一致的状态。
- xv6通过简单的日志记录形式来解决文件系统操作期间的崩溃问题。xv6的系统调用不会直接将数据写入磁盘中的文件系统数据结构，而是将其所要进行的操作作为log写到磁盘中。一旦写入了该条操作所有所需的log，操作系统便会写入一条特殊的commit记录来表明该log包含该条操作所需的所有内容。此时系统调用便会将写操作复制到磁盘上的文件系统数据结构。完成所有的写入后系统调用再将磁盘上的日志擦除。
- 如果系统崩溃，那么在重启以后会根据日志中的内容完成完整的写操作；如果日志中没有包含完整操作（即说明内容要么还没写入磁盘要么已经写完）则恢复代码将忽略该日志。此时恢复代码的工作就是将日志擦除。
- 下面解释xv6的日志是如何解决文件系统操作期间的崩溃问题：如果崩溃发生在操作提交之前，那么磁盘上的日志不会包含完整的操作，也不会有commit，恢复代码将忽略它，并且磁盘的状态将如同操作尚未启动一样。如果崩溃发生在操作提交之后，则恢复代码将重复操作的所有写入操作。在任何一种情况下，日志都会使操作在崩溃时成为原子操作：恢复后，要么操作的所有写入都显示在磁盘上，要么都不显示。
- **日志层的设计**：日志驻留在超级块中指定的位置，其由一个header block和一系列更新块的副本（logged blocks）组成。头块包含一个扇区号数组（每个logged block对应一个扇区号）以及日志块的计数。磁盘上的头块中的计数或者为零，表示日志为空；或者为非零，表示日志包含一个完整的已提交事务，并具有指定数量的logged block。在事务提交（commit）时xv6才向头块写入数据，在此之前不会写入，并在将logged blocks复制到文件系统后将计数设置为零。因此，写入log中途崩溃将导致日志头块中的计数为零；提交后的崩溃将导致非零计数。
